...
This project has custom design tokens defined in `theme.css`. When the guidance below conflicts with project conventions:

- **Use semantic color tokens** (`text-primary`, `bg-secondary`) over raw Tailwind colors
- **Use project typography classes** (`text-body`, `text-title-lg`) over raw `text-sm`, `text-base`
- **Dark mode is automatic** via `light-dark()` CSS function - avoid inline `dark:` variants if using semantic colors from globals.css or theme.css
- **Use `cn()`** from `@/utils/cn` to combine Tailwind classes

See the `tailwind-styling` skill for project-specific tokens and patterns.
...
**In this project**: Use semantic tokens that auto-adapt via `light-dark()` CSS function. Avoid inline `dark:` variants.

```html
<!-- ✅ Project pattern - semantic tokens auto-adapt -->
<div class="bg-primary text-primary">
  <button class="hover:bg-tertiary">Click me</button>
</div>
```
