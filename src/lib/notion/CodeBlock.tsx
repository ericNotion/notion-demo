...
export function CodeBlock({ code, language }: CodeBlockProps) {
  ...
  if (isLoading) {
    return (
      <pre className="bg-secondary max-w-full overflow-x-auto rounded-lg p-4 text-sm">
        <code>{code}</code>
      </pre>
    );
  }
  ...
  return (
    <div
      className="bg-secondary max-w-full overflow-x-auto rounded-lg text-sm [&>pre]:!m-0 [&>pre]:!bg-transparent [&>pre]:!p-4"
