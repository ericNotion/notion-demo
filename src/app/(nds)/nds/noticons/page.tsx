...
import {
  NoticonGroupPreview,
  NoticonVariant,
} from "./components/NoticonGroupPreview";
...
// Load noticon groups only when needed
async function loadNoticonGroups(): Promise<NoticonGroup[]> {
  const [{ noticonManifest }, { noticonImportMap }] = await Promise.all([
    import("@noticons/icon-manifest"),
    import("@noticons/icon-import-map"),
  ]);

  return noticonManifest
    .map((group) => {
      const variants: NoticonVariant[] = group.variants
        .map((variant: string) => {
          const icon =
            noticonImportMap[group.dir]?.[
              variant as keyof (typeof noticonImportMap)[typeof group.dir]
            ];
          if (!icon) return null;
          return {
            label:
        ...
        .filter(Boolean) as NoticonVariant[];
      return {
        name: group.name,
        group: group.dir,
...
function ProductionNotFound() {
  return (
    <div className="bg-main flex min-h-dvh items-center justify-center">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
          404 - Not Found
        </h1>
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Noticons page is not available in production.
        </p>
        <Link
          href="/"
...
// Development noticons page component
function NoticonsPageContent() {
  const [color, setColor] = useState<IconColor>("primary");
  const [weight, setWeight] = useState<IconWeight>("regular");
  const [search, setSearch] = useState("");
  const [noticonGroups, setNoticonGroups] = useState<NoticonGroup[]>([]);
  const [loading, setLoading] = useState(true);

  // Load noticon groups on component mount
  useEffect(() => {
    loadNoticonGroups()
      .then((groups) => {
        setNoticonGroups(groups);
        setLoading(false);
      })
      .catch((error) => {
        console.error("Failed to load noticon groups:", error);
        setLoading(false);
      });
  }, []);

  // Search logic
  const filteredGroups = noticonGroups.filter(
    (group) =>
      search.trim() === "" ||
